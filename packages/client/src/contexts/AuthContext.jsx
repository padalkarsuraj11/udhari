...
export function AuthProvider({ children }) {
  ...
  const value = {
    ...
    signIn: async (credentials) => {
      const { loginIdentifier, email, password } = credentials;

      // Demo login fallback
      if (
        (loginIdentifier === 'PATEL001' || loginIdentifier === 'ramesh@patelelectrical.com' || email === 'ramesh@patelelectrical.com') &&
        (password === 'owner123' || password === 'admin123')
      ) {
        const demoUser = {
          id: 'owner-001',
          ...
          business_name: 'Patel Electrical Traders',
          tenant_id: 'tenant-001',
        };
        localStorage.setItem('client_user', JSON.stringify(demoUser));
        localStorage.setItem('client_token', 'demo-owner-token');
        setUser(demoUser);
        setSession({ access_token: 'demo-owner-token' });
